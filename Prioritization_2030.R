...
# The analysis presented here applies the 2030 LULC projection scenario.
# ============================================================================
...
writeRaster(pc, file.path(output_dir, "2030_Protection_Core.tif"), overwrite = TRUE)
...
writeRaster(rh, file.path(output_dir, "2030_Restoration_Hotspots.tif"), overwrite = TRUE)

# SUSTAINABLE USE AREAS
su <- !pc & !rh
su <- ifel(is.na(su), 0, su); su <- ifel(su > 0, 1, 0); su <- mask(su, study_mask)
writeRaster(su, file.path(output_dir, "2030_Sustainable_Use.tif"), overwrite = TRUE)
...
writeRaster(pm, file.path(output_dir, "2030_Priority_Map.tif"), overwrite = TRUE)
...
calc_stats <- function(study_mask, pc, rh, su, ref, output_dir) {
  ...
  write.csv(stats, file.path(output_dir, "2030_Area_Statistics.csv"), row.names = FALSE)
  
  cat("\n=== AREA STATISTICS 2030 ===\n")
...
create_barchart <- function(area_stats, output_dir, dpi = 300) {
  ...
  ggsave(filename = file.path(output_dir, "2030_Priority_Zone_Area.png"),
